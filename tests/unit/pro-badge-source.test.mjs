...
test("PRO badge is driven by the authoritative server-resolved plan, never client-only session state", () => {
  assert.match(source, /fetch\("\/api\/entitlements\/plan"/);
  ...
  assert.match(source, /userId && fetchedPlan\?\.userId === userId \? fetchedPlan\.plan : null/);
});

test("PRO badge refetches on every auth-state transition, not just on mount", () => {
  const effectBlock = source.slice(
    source.indexOf("useEffect(() => {"),
    source.indexOf("// Only trust a fetched result"),
  );

  assert.match(effectBlock, /\}, \[userId\]\);/);
  // Regression guard: a mount-only effect (empty deps or a stale "run once" pattern) is
  // exactly the staleness bug this component must not reintroduce.
  assert.doesNotMatch(effectBlock, /\}, \[\]\);/);
  // No setState call directly in the synchronous "no user" branch — that's the pattern
  // React's set-state-in-effect rule flags, and the userId-tagged fetchedPlan check above
  ...
  assert.doesNotMatch(noUserBranch, /setFetchedPlan/);
});
